...
jQuery(document).ready(function () {
    ...
    jQuery(
        ".cont_forms.download-pdf input#email"
    ).on("change", function () {
    ...
    jQuery(
        '.cont_forms.download-pdf .con-checkbox input[type="checkbox"]'
    ).on("change", function () {
    ...
    function activateSendButton() {
        if (isInputValid && isCheckbox) {
            jQuery(
                ".js-trigger-send-form"
            ).removeClass("disabled");
        } else {
            jQuery(".js-trigger-send-form").addClass(
                "disabled"
            );
        }
    }

    jQuery(".js-trigger-send-form").on(
        "click",
        function () {
            jQuery(this).hide();
            jQuery(
                ".cont_forms.download-pdf .btn-before-send input"
            ).click();
        }
    );
});
...
function contact_ap_submit(f) {
  ...
  jQuery
    ...
    .done(function (renponse) {
      ...
      jQuery('.con-checkbox input[type="checkbox"]').attr(
        "checked",
        "checked"
      );
...
jQuery(document).ready(function () {
  jQuery('.con-checkbox input[type="checkbox"]').on(
    "change",
    function () {
      console.log(jQuery(this).is(":checked"));
      if (!jQuery(this).is(":checked")) {
        jQuery('.btn-before-send input[type="submit"]').attr(
          "disabled",
          "disabled"
        );
      } else {
        jQuery('.btn-before-send input[type="submit"]').removeAttr(
          "disabled"
        );
      }
    }
...
function refreshCaptcha() {
  jQuery(".captcha").attr(
    "src",
    "../wp-content/plugins/contact-form-with-shortcode/captcha/captcha0a6d.html?rand=" +
      Math.random()
  );
}
...
jQuery(document).ready(function () {
  jQuery("#con-53377-err-msg").hide();
  jQuery("#con-53377").validate({
    errorClass: "rw-error",
    submitHandler: function (form) {
      contact_ap_submit(form);
    },
  });
});

jQuery(document).ready(function () {
  var $anchor = jQuery(".anchor_form"),
    $button = jQuery(".anchor_form").find("button");
  $button.click(function (e) {
    ...
    if (
      jQuery(".sumome-contactform-pullouttab").length >
      0
    ) {
      jQuery(".sumome-contactform-pullouttab").click();
    } else {
...
(function () {
  var wpbrLoader = function () {
    ...
    b.async = 1;
    b.src =
      "https://www.digiteum.com/?gdbc-client=3.1.1-" +
      new Date().getTime();
  ...
  window.addEventListener(
    ...
    function (event) {
      if (event.persisted) {
        typeof window.WPBruiserClient === "undefined"
          ? wpbrLoader()
          : window.WPBruiserClient.requestTokens();
      }
    },
    false
  );
...
jQuery(function (jQuery) {
  jQuery.datepicker.setDefaults({
    ...
    isRTL: false,
      ...
      var ajax_params = {
        ajax_url: "https:\/\/www.digiteum.com\/wp-admin\/admin-ajax.php",
        nonce: "f9b6c008ca",
      };
